...
function Assignexammarks() {
  ...
  const sortMark = async () => {
    const url = 'https://server-six-bice.vercel.app/auth/sortmark/data';
    try {
  ...
  const handlechange = (e, index, rowindex, newvalue) => {
    ...
    if (index === 0 || index === 1) {
      ...
      const url = 'https://server-six-bice.vercel.app/auth/lectupdatemark/inf';
      axios
        .get(url, {
          params: {
            spec: values.spec,
